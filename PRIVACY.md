...
that cannot be private is money of a different kind. The ability to shield funds
is what gives money fungibility and gives users a choice. Nothing is gained by
withholding the option. Most payers will never use it, which is acceptable — the
...
This document maps what Remit leaks, what can be hidden, what it costs, and what
no design here can hide, because "add a privacy layer" is the easiest thing in
this codebase to get catastrophically wrong. The characteristic failure is a name
that promises more than the mechanism delivers, leaving a payer to believe their
payments are confidential when they are not; the defect is in the naming rather
than in the code.

## What the previous version of this document got wrong

It concluded: *"Build nothing that hides amounts."* That was the wrong
conclusion, reached through two specific errors. Both are left on the page, because
a design note that changes its conclusion without recording the change teaches
nothing.
...
That is all true, and what does not follow is that amounts are unhideable on Arc.
Transfers *inside* a shielded pool move no USDC and emit no `Transfer` at all, so
amounts are hideable, though not by a flag bolted onto this design.

**Error two: stealth addresses were rejected with an argument that does not
...
thing a paymaster sponsors. That amounts to friction and a real unsolved piece of
work rather than a disqualification.
...
Remit places no constraint on who the payer is. Any address that can call
`approve` on USDC and `createMandate` on Remit can be a payer, and a contract can
do both.

**Privacy is therefore a payer rather than a flag.**

That distinction carries further than it first appears, because `MandateManager`
is immutable, with no proxy, no admin, no owner and no upgrade hatch, all of it
deliberate, so that no key can raise a cap or drop an allowlist after a payer has
granted. A privacy feature that required changing the contract would mean a
redeploy that orphans every live mandate and invalidates every gas measurement
taken against the current bytecode. A privacy feature that sits *above* it, as a
payer, changes nothing, spends no flag bits, and adds nothing to the audited
surface of the one contract that moves money.
...
**Storage is not secret — on the public EVM.** Restricting `getMandate` to `msg.sender`
would hide nothing there: contract storage is world-readable with `eth_getStorageAt`, and
this repository documents the slot layout. An EVM contract on a public chain cannot keep a
secret it has stored — only one it never stored. Every layer below is therefore a
commitment or a proof rather than an access-control check.

**That is an environmental fact rather than a law, and Arc intends to change it.** The
...
today's public EVM, and it does not generalise beyond that environment.
...
A fifth architecture exists that Remit does not own and that is not available yet:
**Arc's own Privacy Sector.** It bears on the same question as the table without fitting
inside it, and it is described after the four layers because it would change what they are
...
The commitment occupies the same 32-byte word as a plaintext label, though **not**
the same gas, and the earlier draft of this section claiming a zero delta was wrong.
Calldata is priced per byte — 16 for a non-zero byte, 4 for a zero one — and a
keccak digest has no zero bytes at all, so the commitment's word costs 32×16 = 512
gas, while `"invoice-0002"` zero-padded to 32 bytes costs 12×16 + 20×4 = 272. **The
...
L0 remains the least expensive measure in this document by a wide margin — no
contract change, no new storage, no audit surface, and chosen per *spend* by the
caller — while still costing a number rather than a zero, a distinction that matters
here for the same reason it mattered twice before in this file.

Two boundaries apply: the salt is mandatory, since without it a guessable preimage
like `invoice-0002` is brute-forced in microseconds; and a vendor reconciling
...
L1 is now committed rather than undecided, and it spends **bit 7**, the last free
bit in `uint8 flags`.
...
the incremental leak is zero. Merkle proofs are conventional and battle-tested,
with no trusted setup and no novel assumption. Eight vendors is a three-level
proof: 96 bytes of calldata, roughly 1,536 gas, plus three hashes, against a
177,429-gas baseline.
...
`F_ALLOWLIST` with a non-empty list; its own tests added to the current 254; and a
...
linked to the vendor without their viewing key. The scheme requires no ZK, no
pool and no new trust assumption, and funds still never touch Remit.

Three problems remain, none of them fatal.
...
is fine, because the delegate pays. The sweep is the open problem, and it is
deferrable, batchable across many addresses in one transaction, and sponsorable;
the EIP-7702 question already open in the backlog bears directly on it.

*Allowlist compatibility.* Commit the meta-address and have the delegate prove
derivation at spend time, which is why L1 and L2 compose.

*Cost.* Every stealth address is a first-time USDC holder, and this repository's
own measurements put the account-creation premium at roughly 5,333 gas — on every
payment, not once.
...
L3 is a separate contract: depositors put USDC in and receive a commitment in a
merkle tree. To authorise spending, a depositor submits a proof that they own an
unspent commitment worth at least the amount, and the vault — as payer — grants the
...
`MandateManager` is untouched, because it is immutable and needs no change.

Three hazards are named now so they are designed against rather than discovered:
...
1. **The two-layer cap hazard.** Remit's cap is per mandate; the vault's cap is
   ...
   atomically with the spend"*, and **that is unimplementable**; the retracted
   sentence is retained here alongside its correction. A spend runs `agent →
   MandateManager.spend → usdc.transferFrom(vault → recipient)` with no vault code
   executing, since ERC-20 offers the token holder no hook and `spend` requires
   `msg.sender == m.spender`, so the vault cannot interpose itself without becoming
   the agent. The second half of the old sentence was the whole answer — escrow at
   grant time, size `totalCap` to the nullified commitment, reclaim the remainder
   from the public `totalSpent`, as specified in `L3-VAULT.md`.
...
2. **The delegate as pseudonym.** `MandateCreated` and `Spend` are both indexed
   ...
   Delegates have to be one-time, or shared in practice.

3. **Amount privacy needs the other side in the pool.** A vault-to-external
   spend still publishes amount and recipient, and only vault-*internal* transfers
   ...
   network-effect problem rather than a cryptography problem, and users should be
   told which one they are getting.

The costs are real: funds enter a contract, so non-custody holds for the base layer but
not for depositors who opt in — that is the trade they are choosing, and it should be
presented as a choice rather than hidden. **Revocation is part of that trade, and an
earlier version of this paragraph omitted it:** `revoke` admits the payer or the
spender (line 704), and under L3 the payer is the vault, so a depositor cannot revoke
their own mandate while the operator can revoke any depositor's. Remit's headline
control is transferred to the pool unless the circuit is designed to give it back — see
`L3-VAULT.md`. Circle's freeze authority compounds the same way: freezing the vault
freezes every depositor at once, so pooling converts individual freeze risk into shared
fate. A ZK vault also means a circuit, a verifier, nullifier handling, and either a
trusted setup or a transparent proof system, which is a project with its own timeline
rather than a sprint. An anonymity set of one is not an anonymity set — the set is the
number of *unreleased mandates* the vault holds, not the number of depositors, so the
earliest depositors get the least privacy, which is true of every shielded pool ever
built and must be disclosed rather than discovered.
...
`GAS-ABSTRACTION.md` for how it surfaced and the documentation rule that followed.
...
Arc."*** No date accompanies it, so nothing here is buildable today, and everything
below concerns sequencing rather than switching.

APS changes three things about this document.
...
are encrypted state under APS, at no cost in Solidity. Amounts and recipients would
follow, if a confidential USDC representation exists on the private side. That last
point is an open question the documentation does not answer, and it is the crux:
...
hardware enclaves, a validator-held master key, and an attestation policy. Enclave
compromise and side channels are real; so are circuit bugs and trusted setups. The
accurate framing is that these are different failure modes for the same property, and a
payer might reasonably prefer either — which is an argument for eventually offering
both, and against pretending the choice is obvious.
...
payment, and APS would deliver that as a deployment target. The spec's *contract*
findings are permanent and justified the session regardless, because they are facts
about `MandateManager` rather than about cryptography. Committing months of circuit
engineering without knowing APS's timeline would be a mistake, and asking Circle before
building is the inexpensive step.

One detail carries over even if Remit never deploys into APS: **constant-time gas
...
- **Whoever pays gas leaks**, and on Arc gas is USDC, so the gas payer is visible
  in the same asset as the payment. For L2 and L3 this is a decisive dependency
  rather than a marginal correlation risk: a depositor who submits their own
  ...
  hides. Gas abstraction is a prerequisite for both layers rather than a refinement
  of either; see `L3-VAULT.md`.

  **The mechanism for it already exists on Arc, which an earlier version of this file
  implied it did not.** ERC-4337 with EntryPoint v0.7 and a USDC-funded paymaster is live
  on testnet and documented, so the grant can originate from a smart account rather than
  the depositor's own key. What that buys is real but bounded: **the leak moves to the
  sponsor.** A depositor's privacy set becomes whoever shares their paymaster or relayer,
  and Arc names a single third-party bundler as the documented path — so day-one L3 has
  one party who sees every grant request. The accurate word is "relayed" rather than
  "trustless"; see `GAS-ABSTRACTION.md`.
- **Nothing is retroactive.** Every transaction already on chain — including the
  project's own four live spends and their plaintext `invoice-000N` refs — stays
  public forever.

## Naming

The bit must never be called `F_PRIVATE`; name the mechanism instead, since
`F_ALLOWLIST_ROOT` describes what the bit does and promises nothing about outcomes.
A user who toggles something called "private," believes their payments are
confidential, and is wrong has been harmed by the name rather than by the code.
...
## The regulatory question, which is for counsel
...
policy for Remit: who sponsors, how recipients are screened before a blocklisted address
...
**The open question that now outranks all of it.** Arc's own confidential execution
...
Build the option, name it accurately, and claim only what each layer supports.
