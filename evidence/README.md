...
`reference/mutation-gate-sol.py` deletes one `revert` statement at a time, rebuilds the
project in a throwaway copy and re-runs the suite, so a mutant no test notices is a guard
with nothing standing behind it. The JS mutation gate's log stays in the Toolchain table
below with the other whole-suite baselines, because it measures `reference/policy.js`. Every
file here except the census is superseded by it and kept anyway: each records a run at a
suite size no later run can reproduce, and the sentences in `THREAT-MODEL.md` that lean on
them describe what one dated run showed rather than what the tree does today.

| File | What it establishes |
|---|---|
| `mutgate-sol-census-320.log` | **Every target on the tree at `72d46ca`, run 2026-09-05: 123 mutants over 17 invocations, 119 caught by a named test, 4 EQUIVALENT, 0 SURVIVED, 0 INCONCLUSIVE, and `baseline: 320 passed, 0 failed` on all 17.** The split is 103 removals, 6 injections and 14 hand cases, which closes against the enumeration `THREAT-MODEL.md` §5 carries. Thirteen bare runs cover 109 mutants across thirteen targets and four `--hand` runs cover the other 14, two of them for helpers that hold no `revert` and so offer hand cases and nothing else. The four equivalent mutants are `CredentialMissing` in `_checkCredential`, `UnknownMandate` in `spendableAcross`, `_cosignIsLive`'s empty slot and F44's mask written as a magnitude test; each prints the gate's widening reason in place of a verdict instead of being counted as caught. Two targets appear twice, once bare and once hand-only, and each hand-only run closes with the gate's own note that the removals were not built, so it is not a census of that target. The file is also the proof that this gate runs at all: `COPY` had omitted `script`, `test/Deploy.t.sol` imports `../script/Deploy.s.sol`, and every target had been failing at exit 2 with zero mutants since that test file landed. |
| `mutgate-checkIdentity.log` | **`_checkIdentity`'s 2 mutants, run 2026-08-29 at `baseline: 207 passed, 0 failed`: both caught.** `IdentityNotHeld` dies to three tests and `IdentityTransferred` to one. The census re-establishes both verdicts at 320, same errors and same killers, so this file is kept for its control line rather than its result. `THREAT-MODEL.md` cites the 207 as the provenance of a suite figure, and a run at 320 cannot back a sentence about what the suite held in August. |
| `mutgate-only-approveCosignFor.log` | **The confirmation that `UnrecoverableRecipient` in `approveCosignFor` is asserted by something, run 2026-08-30 at `baseline: 209 passed, 0 failed`: the removal caught by `test_f29_approvingTheContractOrTheTokenAsRecipient_isRefused`.** It ran under `--only`, and its first line records that the target's 4 injections were skipped, so it is a confirmation rather than a census. What it holds that no later run can is the repair: the full 24-mutant sweep the same day reports 24 of 24 and shows none of this, because the test was already in the tree when it ran. A survivor's repair is visible only in a run that predates the repair, or in the `--only` run written afterwards to recover the evidence. |
| `mutgate-only-createMandate.log` | **The same shape for F32, run 2026-08-30 at `baseline: 209 passed, 0 failed`: the credential guard's `BadConfig` removal caught by `test_createMandate_eachCredentialFieldWithoutTheFlag_isRefused`.** The guard had been shadowed rather than unasserted: the biconditional one block earlier reverts the same error, so the test that appeared to cover it was refused before reaching it, and the gate deleted the guard while all 207 tests still passed. This log is the run that shows the new test reaching the guard itself. The census re-runs all 31 of `createMandate`'s guards at 320 and catches every one, on a tree whose line numbers have since moved. |
| `mutgate-sol-spend-inject.log` | **`spend`'s one injection under `--injections`, run 2026-08-30 at `baseline: 212 passed, 0 failed`: caught.** The gate labels the mutant "injected F22: the spender refused as its own recipient", and its killers are `test_f22_theSpenderMayBePaidByItsOwnMandate`, `test_f22_anAllowlistIsWhatStopsTheSpenderBeingPaid` and `test_f22_approvingTheSpenderAsRecipient_isAccepted`, at `[3 failing]`. The closing `SCOPE: injections only` block records the 19 removals the run declined to build, which is how the log says of itself that it is not a census. The census re-runs the same injection at 320 and catches it there too, on a tree where removals and injections together build 21 for the target rather than 20. What this file holds is the 212 baseline `THREAT-MODEL.md` reports, and no later run can reproduce it. |
| `mutgate-sol-approve-inject.log` | **`approveCosignFor`'s five injections at the same baseline, run 2026-08-30: 5 of 5 caught.** Four are F17 cases already in the tree, so their result is a regression check; the fifth is F22 on the approval path, killed by exactly one test, `test_f22_approvingTheSpenderAsRecipient_isAccepted`, at `[1 failing]`. That single killer is what the paragraph leaning on this log records, because deleting that one case would let the mirror injection survive a green suite. Its `SCOPE` block names the 20 removals skipped. The census carries all five injections at 320 with the same verdicts, on a tree where removals and injections together build 27 for the target rather than 25. |
...
| `lint-final.log` | **The 2026-08-30 `forge lint` result, one line long: `No files changed, compilation skipped` and nothing else.** A clean check prints nothing, so what this file establishes is the absence of any `warning[...]` block. The reason that reads as a clean tree rather than a lost stream is that the sibling captures from the same session hold full `warning[...]` blocks, so stderr was being read all along — and those siblings stayed local, which is the one part of the argument a clone cannot check. It has since gone stale the way a clean checker log always can: F45's guard arrived in `9b701ee` on 2026-09-01 as the contract's seventh `block.timestamp` comparison and the only one without a pragma beside it, and the next run reported it. `FORGE.md` carries the method, the annotation that answered it, and the rule that a clean checker log dates from its run. |
...
| `mutgate-js-all.log` | **`reference/mutation-gate.js` against every throw-bearing function in `reference/policy.js`, run 2026-09-05: 105 mutants over eleven targets, 105 caught, 0 survived, 0 inconclusive, exit 0 eleven times.** The split is 94 removals and 11 injections, and the totals are checkable two ways — each target's own header declares its count, and the summed headers agree with the counted `caught` lines. Read what this log is *not* before quoting it: the gate had only ever been pointed at five of those eleven, and the first run across all of them found five survivors — `window` at `reference/policy.js:238`, 239 and 241, `commit` at 1033, `headroomAcross` at 1673 — every one of them simply unasserted rather than shadowed or equivalent. This run is the one *after* the four tests that kill them, so it records the closed state and cannot show the holes; the discovery is in `CHANGELIST.md` and in git history at `1a27a93`, where the same eleven targets return five survivors against 112 tests. It is also the only `mutgate-*.log` in the Toolchain table; the Solidity counterpart is `mutgate-sol-census-320.log`, in the mutation-gate section above. Neither stands in for the other, since that gate mutates `contracts/MandateManager.sol` and this one mutates `reference/policy.js`. |

**Two of these ten files are the reason this folder exists rather than a sentence in a
