...
| `coverage.log` | **`forge coverage` at 278 tests, run 2026-09-04: the contract at 100% of lines (313/313), statements (565/565), branches (144/144) and functions (27/27), and 99.84% / 98.55% / 92.65% / 100.00% across the eight files the report reaches.** It is the only backing anywhere for the coverage figures quoted in `README.md`, which is the whole reason it is here. Read its first line before setting it beside anything else in this folder: `forge coverage` disables the optimizer and `viaIR` to keep its line mapping honest, so these numbers describe a different compilation from the one the deep gate runs and the one deployed on Arc Testnet. |

**Two of these six files are the reason this folder exists rather than a sentence in a
