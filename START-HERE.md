...
one is written for someone who doesn't yet, and it assumes nothing.

## Where you are

**All five stages below are done, as of 2026-08-24.** Node is installed and the reference
tests pass — 57 of them in the current tree, since v2 has been adding to that suite. Foundry
is installed under WSL, the contract compiles, and all 140
...
without ever holding your key.** Stage 5 got done before stage 4 because it needed nothing
but the local suite.
...
**What's next is no longer a stage in this guide.** It's the pre-audit work listed at the
end of README.md: the deep test profile, exercising the paths testnet hasn't touched
(cosignature, the two ERC-8004 gates, revocation), and an audit — because this is meant to
hold real money.
...
browser. It works right now, needs nothing installed, and it's the fastest way to see
what you've built. Double-click it.

The **documents** explain the reasoning, the threat model, and what the project does
*not* protect against. They're honest about the gaps, which is the main thing that
...
nothing needs copying back and forth. Everything below assumes that's where it is. Inside
WSL the same folder is `/mnt/c/Users/DELL/projects/remit`.

## The decision that changes the plan — answered

**What do you want Remit to be?** There were two honest answers. As of 2026-08-24 you've
picked the second one: **something that holds real money.**

*A thing you built and can show people* was the other option — deployed to Arc's test
network, where the money is fake and free, with a working demo and documentation
explaining why it exists. Nobody needs to audit that, because nobody can lose anything.

*A thing that holds real money* is the same code and an entirely different bar. Three
...
that realistically costs tens of thousands of dollars and takes weeks. I'd tell you not to
skip it, and I'd tell someone with five years of Solidity behind them exactly the same
thing — contracts that move other people's money get attacked by people who do this full
time, and they don't grade on a curve for a first project.
...
setting for development and the wrong one for a final gate.

And the three "documented soft spots" in the README stop being curiosities. The one I'd
revisit first is a mandate configured with `perTxCap` below `cosignThreshold`: it silently
...
next. Repeat until it's green.

Your job in that loop is to be an accurate messenger. Don't summarise error messages
and don't leave out the boring lines; the boring lines are usually the ones with the
file name and line number in them. If a command produces two hundred lines of output,
paste two hundred lines.

Two habits worth having from day one, and stage 4 has now put both to work. Never paste a
private key or a seed phrase to me or to anyone else, in any context, ever — the safe way
is Foundry's encrypted keystore, which is what your two testnet wallets use. And when a
tool asks whether you want to do something you didn't expect, read it before agreeing.
...
You'll need a terminal. If you have **Visual Studio Code**, use the one built into it:
...
project. That's the easiest option, because the code and the terminal are in the same
...
If PowerShell says it doesn't recognise `node`, close it and open a new one — the
...
it defeats. If you see `57 pass`, the foundation of the project is sound and you've run
...
three for the co-signature gate holes; six for the joint-ceiling view; and one for the
narrowed definition of a bounded mandate — a per-transaction cap or a window is no longer
enough on its own, because neither limits what can be spent over a lifetime. This number
...
which meant the project was broken — they meant nobody had ever proofread it
...
WSL, which is a real Linux system running alongside Windows. I can't check Foundry's
current native-Windows support from this environment, so I'm pointing you at the route
I'm confident about rather than the one that might be shorter.
...
that password is for Linux, and it's normal that nothing appears on screen while you
type it.

From then on you'll work in the **Ubuntu** terminal, not PowerShell. Inside it, your
...
than errors. There were 91 of them. They have since been read one at a time: five in the
contract now carry a written explanation of why that particular line is sound, three real
problems in the test harness were fixed, and the rest were a category the compiler already
checks. `FORGE.md` has the whole account. The reason it's worth mentioning to you is the
lesson rather than the detail — a warning you have decided to ignore stops being read, and
...
deliberately pinning behaviour that *looks* like a bug and isn't.
...
account with no USDC can't do anything at all, not even fail — and that applies to the
...
Six of the policy gates were then re-checked against the live contract for zero gas, by
...
doesn't exist, and — the important one — replaying the payment that already went through.
Each was refused with exactly the error predicted. The replay refusal is the one that
matters, because it's the difference between a duplicated invoice and a rejected one.
...
More slices means more accurate rate limiting and a more expensive transaction, and
nobody knew where the line was. The command turned that into arithmetic: **a median spend
...
accurate to 96%. It isn't close. The generous setting is the right default. `DESIGN.md` has
the full table and the three conditions on believing it.
...
The audit is no longer on this list, because you've said this will hold real money. It is
...
going to be a showpiece they affected how you'd *describe* Remit. Now one of them —
...
no cosigner, no agent-identity check, no credential check, a single spending window, a
single approved recipient. Everything else in the contract has passing tests and zero real
transactions behind it. The mandate you granted is still live with 1.90 of budget left, so
those paths can be exercised without granting anything new. That's the most useful next
piece of work in this folder.

## When you get stuck

Paste the command you ran and everything it printed. Tell me which stage you're on.
That's enough — I have the whole project in front of me and I don't need you to
diagnose anything.

If this stops being fun, say so. You've said this is meant to hold real money, which is a
legitimate goal and also the demanding version of the project — so it's worth knowing that
a working testnet deployment with good documentation is a complete, respectable thing to
have built on its own. You now have exactly that. It is a fine place to stop or pause if
you decide to, and everything from here on is optional in a way that nothing before it was.

## Small glossary

The other documents use these without explaining them.

A **contract** is a program stored on a blockchain that anyone can call and nobody can
secretly change. **Solidity** is the language it's written in. To **compile** is to
turn that source code into something the blockchain can execute, and it's also the step
...
that's itself a contract. An **allowance** is the standard permission slip letting one
