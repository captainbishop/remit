...
Research note on the prerequisite `L3-VAULT.md` identified in front of both remaining
privacy layers, recorded as Task #5. Sourced entirely from Arc's own documentation via the
arc-docs MCP; every claim below cites the page it came from, and the section at the end
lists what the docs do **not** say, because what the documentation omits matters as much
as what it states.
...
**Headline: gas abstraction on Arc is three mechanisms rather than one, and the one that
fits Remit is the one this research was not looking for.** Separately, it turned up a
documented Arc feature, absent from every document in this repository, that changes the
privacy roadmap materially; the final section covers it and outranks the rest of this note.
...
Two operational facts from that page apply regardless, because they are about Arc rather
than about EIP-3009. A `transferWithAuthorization` that **fully drains a
...
premium. A blocklisted `from` or `to` **reverts at runtime and consumes the
...
Arc documents EntryPoint **v0.7** at `0x0000000071727De22E5E9d8BAf0edAc6f37da032`, with the
hedge *"verify the EntryPoint v0.7 address for Arc testnet before use"* on its own page, so
the address was verified, on 2026-08-25, against Arc testnet:
...
`0x07f96d30b0cc2f1c5f6381309cbc63d3c80eb361a777ff3b7d29ea473e06c930`, and Arc returned:
...
rather than a liveness one: the selector `0x22cdde4c` is
`getUserOpHash(PackedUserOperation)`, v0.7's nine-field struct with its two packed
`bytes32` gas words, while v0.6's `UserOperation` is eleven fields and hashes under
`0xa6193531`, a different function that this call would not have reached; and the returned
value commits to `address(this)` and `block.chainid`, so the same bytecode at another
address, or on another chain, could not produce this number.

**The limit that remains.** This proves the *hashing path* is v0.7's, which is precisely
...
passes through EntryPoint, and executes from the **smart account**, which has two
consequences:
...
- For a **grant**, `createMandate` records `payer: msg.sender` (line 371). Under 4337 that
  is the smart account, not the human — which for L3 is *exactly right*, since the
  vault is the payer anyway.

4337 is therefore usable, though it remains a decision about **what type of account the
agent and the vault are** rather than a transport that can be added afterwards.
...
EOA is still an EOA: it keeps its own address as `msg.sender`, `tx.origin` still equals
it, and it has code, so for Remit's own `spend` and `createMandate` paths, a delegated
EOA satisfies the spender and payer checks natively — no special handling, nothing to
change in v2.
...
documented reasoning suggests it should pass. That is an inference rather than a documented
fact, and inferences of exactly this shape have misled this project before. **It needs one
...
`from`, making an unsponsored L3 worse than no L3. That conclusion stands, and the fix
requires nothing to be built on Arc — 4337 with a paymaster is live *today*, and
under it the grant originates from a smart account rather than the depositor's EOA. The
remaining work is a relayer or paymaster policy for Remit rather than a missing chain
feature. **The path is open; it was never closed.** An earlier reading described it as the
next piece of work without checking whether the chain already offered it, inferring a chain
limitation from the fact that Arc's tutorials use a plain EOA.

**2. The relayer-sees-who-asked caveat becomes sharper and more severe.** Under 4337, the
...
**3. Blocklist reverts burn the sponsor's gas.** Arc documents this for the 3009 path, and
it holds in general on Arc. A Remit paymaster sponsoring spends to arbitrary recipients
pays for every blocklisted-recipient revert, so recipients must be screened before
sponsoring, or the sponsor becomes a free denial-of-wallet target. This is a new
operational requirement that neither `PRIVACY.md` nor `L3-VAULT.md` anticipated.
...
~230,000 through ring buckets and effectively unbounded through the credential check's
decoded string. Under sponsorship, that inflation is charged to the sponsor, and the caps
that spec recommends for a vault are equally necessary for a paymaster, for a different
reason.

## What the docs do not say

Recorded so that no reader later mistakes absence for confirmation.

- **No `permit` / EIP-2612 on Arc's USDC.** `rg` found `permit` only in App Kit pages, and
  `receiveWithAuthorization` and `cancelAuthorization` appear **nowhere** in the docs,
  despite both being part of standard EIP-3009. Whether Arc's USDC implements them is
...
- **The EntryPoint address is explicitly flagged as needing verification** by Arc's own
  ...
  a locally predicted `getUserOpHash` exactly, over v0.7's `PackedUserOperation` ABI, with
  the derivation in `userophash.py` and the log in `evidence/entrypoint.log`. What
  remains formally open is whether the *whole* bytecode is byte-identical to the canonical
  release, which needs a reference codehash this environment cannot fetch; the hashing path
  that signature compatibility rests on is confirmed.
...
- **Nothing on whether a 7702-delegated EOA passes `Memo`/`CallFrom`.** This rests on
  inference alone, per above.
- **The 20 Gwei floor appears twice** (`maxFeePerGas` minimum, "transactions priced under
  ...
  on Arc receipts. That is consistent rather than contradictory, because a floor is not a
  price, and the measured number is the one to cost with.
...
itself into a confidential environment with no cryptography of Remit's own — the exact
outcome `PRIVACY.md`'s leak inventory marks as hideable "only at ruinous cost". It is a
different trust model (hardware enclaves and a validator-held master key, versus a proof
anyone can verify), and that trade deserves to be argued on the page rather than omitted
from it.
...
depositor authorised a payment, and APS would deliver that as a deployment target. The
spec retains its value, because its contract findings are permanent and an enclave-based
answer is not strictly better than a ZK one, while building L3 without knowing APS's
timeline would be a mistake, and "not yet available" with no date is not a timeline.

**Two privacy layers were specified against this chain without reading its privacy page.**
The search that found APS took one query. `PRIVACY.md` opens by saying privacy is the
easiest thing in this codebase to get catastrophically wrong; getting it wrong by not
checking whether the platform already has an answer was not the failure mode being watched
for. The rule that follows: **before specifying a capability for a specific chain, read
that chain's own documentation on the capability.** The arc-docs MCP was available for both
prior sessions.

**Two APS details are directly useful even if Remit never deploys into it.** Constant-time
gas reporting to external observers is an acknowledgement, by Circle, that *gas is a side
channel* — which supports the finding in `L3-VAULT.md` that depositor-controlled spend gas
is an information leak and not merely a cost. Default-deny with per-selector policies is
a cleaner formulation of what `PRIVACY.md` correctly rejects for the public EVM
("storage is not secret", so access control hides nothing): under APS, access control
*does* hide something, because storage is encrypted. The reason `getMandate` cannot
restrict who reads it today is environmental rather than a law.

## Recommended next steps

Each of these is cheap, and each settles a question rather than adding surface:
...
1. ~~`cast code` the EntryPoint, then confirm it is really v0.7.~~ **Done 2026-08-26.**
   ...
   v0.7's ABI, recorded in `evidence/entrypoint.log` and `userophash.py`. Nothing further
   needs spending here: the remaining gap is whole-bytecode canonicality, which needs a
   reference this environment cannot fetch and which a paymaster integration does not
   depend on.
2. **Test whether a 7702-delegated EOA passes `Memo`.** This settles the `CHANGELIST.md`
   question with a receipt instead of an inference, and needs a delegation and one memo call.
3. **Ask Circle about the APS timeline** before committing engineering to L3. "On the
   roadmap" with no date is a reason to sequence rather than to stop.
